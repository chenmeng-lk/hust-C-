#include"ex2.h"
QUEUE::QUEUE(int m) :elems(new int[m]), max(elems?m:0) //初始化队列：最多申请m个元素
{
    head = tail = 0;
}
QUEUE::QUEUE(const QUEUE& q) :elems(new int[q.max]), max(elems?q.max:0)//用q深拷贝初始化队列
{
    head = q.head;
    tail = q.tail;
    for (int i = 0; i < max; i++)
...
QUEUE::QUEUE(QUEUE&& q)noexcept :elems(q.elems), max(elems?q.max:0)	//用q移动初始化队列
{
    head = q.head;
    tail = q.tail;
    *(int**)&q.elems = nullptr;
...
char* QUEUE::print(char* s) const noexcept //打印队列至s并返回s
{
    ...
    for (int i = head; i != tail; i = (i + 1) % max) {
        index += sprintf(s + index,"%d,", elems[i]);
    }
    if (index > 0) {
