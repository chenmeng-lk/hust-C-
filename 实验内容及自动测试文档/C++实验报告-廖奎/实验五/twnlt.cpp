...
void twnlt::closewnd() {
	if (ui.action_exit->isChecked() == false)return;//鼠标单击触发两次，第二次直接返回
	ui.action_exit->setChecked(false);//第一次触发设置状态为false，防止第二次触发进入
	if (fl != Q_NULLPTR) {
		fl->hide();
		delete fl;
		fl = Q_NULLPTR;
...
void twnlt::zdcx() {
	...
	if (ls != Q_NULLPTR) {  //如果先前打开过站点及线路输入窗口
		ls->show();         //则直接显示该窗口
		return;
	}
...
void twnlt::loadmap() {
	...
	arrive = depart = false;//此时未选取步行起点或步行终点
...
void twnlt::zszc() {//先获得起点坐标和终点坐标，并计算最少转乘的路径
	if (ui.action_zszc->isChecked() == false)return;//鼠标单击触发两次，第二次直接返回
	ui.action_zszc->setChecked(false);//第一次触发设置false，防止第二次触发进入
	QList<QGraphicsItem*> listItem;
	if ((depart && arrive) == false) return;//若没有选中步行起点和步行终点，则返回
	...
	QBrush qbrush(QColor(100, 100, 250));//设置颜色
	QPen qpens(qbrush, 4, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin); //站点的笔
	QPen qpenl(qbrush, 3, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin);//线路的笔
	route = true;//进入查询路径显示期间，不响应步行起点与步行终点选取
	int c, n = 0;//c为可行转乘方案数，n为最少转乘次数
	ROUTE r[100];//一次查询，最多返回100条可行转乘方案
	for (int d = 0; d < itemDepart->c(); d++) {//接近步行起点的所有公交站点
		int s = (*itemDepart)[d]; //获得起点站点编号s
		for (int a = 0; a < itemArrive->c(); a++) {//接近步行终点的所有公交站点
			int t = (*itemArrive)[a];//获得终点站点编号t
			if (s == t) {//起点站和终点站相同，不用转乘
			...
			for (int m = 0; m < c; m++) {//对于第m个转乘方案，即route=r[m]r［m］，转乘次数都为n
				deleteItems();
				gItem = new QGraphicsItemGroup();
				myEItem = new QGraphicsEllipseItem(itemDepart->x(), itemDepart->y(), 6, 6);
				myEItem->setPen(qpens);
				...
				int bg, ed;//起始站点序号bg，与终止站点序号ed
				myLItem = new QGraphicsLineItem(itemDepart->x(), itemDepart->y(),
					gis->st[s].X(), gis->st[s].Y());
				myLItem->setPen(qpenl);
				gItem->addToGroup(myLItem);//到起点站
				if (n == 1 && r[m][0].S() == -1) {//从i路到i路（此时S0 = -1) - 1）不用转乘
					fm = r[m][0].F(); //已乘线路序号fm
...
void twnlt::zdjl() {//先获得起点坐标和终点坐标，并计算最短距离的路径
	if (ui.action_zdjl->isChecked() == false)return;//鼠标单击触发两次，第二次直接返回
	ui.action_zdjl->setChecked(false);//鼠标第一次触发设为false，防止第二次触发进入
	QList<QGraphicsItem*>listItem;
	...
	for (int d = 0; d < itemDepart->c(); d++) {//接近起点的所有公交站点
		...
		for (int a = 0; a < itemArrive->c(); a++) {//接近终点的所有公交站点
			...
			for (int m = 0; m < c; m++) {//第m个转乘方案，即route = r[m]，最短距离都为 dist
				deleteItems();
				gItem = new QGraphicsItemGroup();
				myEItem = new QGraphicsEllipseItem(itemDepart->x(), itemDepart->y(), 6, 6);
				myEItem->setPen(qpens);
